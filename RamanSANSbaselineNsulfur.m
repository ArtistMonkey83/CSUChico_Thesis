...
% Two figures: one with sulfur and 273-240 baseline subtracted and one as collected
...
for k = 1:length(files)
    ...
    % Interpolate sulfur reference
    y_s8_interp = interp1(x_sulfur, y_sulfur, x, 'linear', 'extrap');
    y_sub = y - y_s8_interp;

    % --- Accumulate y between 273 and 340 cm^-1 ---
    mask = x >= 273 & x <= 340;
    x_masked = x(mask);
    y_masked = y(mask);
    ...
    % --- Subtract average and normalize ---
    y_sub_adjusted = y_sub - yAvg;
    y_norm_sub = minmax_norm(y_sub_adjusted);
