...
% One figure just sulfur subtracted results
...
for k = 1:length(files)
    ...
    y_sub = y - y_s8_interp;
    y_norm = minmax_norm(y_sub);

    % Offset
    offset = (k-1) * offsetStep;

    % Extract label: string between 1st and 3rd underscore
...
titleStr = 'Glow Grid 2.5 \muM: Battery A (Sulfur-Subtracted Raman Spectra)';
h_title  = title(titleStr);
h_xlabel = xlabel('Raman Shift (cm^{-1})');
h_ylabel = ylabel('Normalized Intensity + Offset');
